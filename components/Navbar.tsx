  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-md border-b border-white/10' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0 flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo(0,0)}>
            ...
            <span className="text-white font-semibold text-lg tracking-tight">D'click Studio</span>
          </div>
      ...
      <div className={`md:hidden ${isOpen ? 'block' : 'hidden'} bg-black/95 backdrop-blur-xl absolute w-full`}>
        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 border-b border-white/10">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
